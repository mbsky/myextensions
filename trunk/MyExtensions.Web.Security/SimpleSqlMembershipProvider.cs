    ...
    public sealed class SimpleSqlMembershipProvider : MembershipProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (String.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "MyExtensions membership provider");
            }
        ...
        {
            get { return System.Convert.ToBoolean(this.GetConfig("requiresQuestionAndAnswer", "false")); }
        }
        ...
        public Md5Length Md5Length
        {
            get
            {
                return (Md5Length)Convert.ToByte(this.GetConfig("md5Length", "16"));
            }
        }

        public bool CheckOldMD5Password
        {
            get
            {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("usp_Users_Insert", db))
                {
                    ...
                    cmd.Parameters.Add("@Question", SqlDbType.NVarChar, 100).Value = passwordQuestion;
                    cmd.Parameters.Add("@Answer", SqlDbType.NVarChar, 100).Value = passwordAnswer;
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            try
            {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand(getUserSql + " WHERE u.UserName=@UserName", db))
                {
                    cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username;
        ...
        public override MembershipUser GetUser(object providerUserKey, bool userIsOnline)
        {
            ...
            try
            {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand(getUserSql + "WHERE u.UserId=@UserId", db))
                {
        ...
        public override bool ChangePasswordQuestionAndAnswer(string username, string password, string newPasswordQuestion, string newPasswordAnswer)
        {
            ...
            {
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("Update UserSecurity Set Answer=@Answer,Question=@Question FROM UserSecurity as s Inner Join Users u on u.UserId = s.UserId WHERE u.UserName=@UserName", db))
                {
                    cmd.Parameters.Add("@Answer", SqlDbType.NVarChar, 100).Value = newPasswordAnswer;
                    cmd.Parameters.Add("@Question", SqlDbType.NVarChar, 100).Value = newPasswordQuestion;
                    cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username;
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            try
            {
                using (HostingEnvironment.Impersonate())
                {
                    using (SqlConnection connection = this.OpenDatabase())
                    {
                        string cmdText = "SELECT s.PasswordHash, s.PasswordSalt";
                        if (this.CheckOldMD5Password)
                        {
                            cmdText = cmdText + ",s.MD5PasswordFlag";
                        }
                        if (this.RequiresUniqueEmail && IsEmail(username))
                        {
                            cmdText = cmdText + " FROM UserSecurity s INNER JOIN Users u ON s.UserId = u.UserId WHERE u.Email=@UserName AND u.Enabled=1";
                        }
                        else
                        {
                            cmdText = cmdText + " FROM UserSecurity s INNER JOIN Users u ON s.UserId = u.UserId WHERE u.UserName=@UserName AND u.Enabled=1";
                        }
                        using (SqlCommand command = new SqlCommand(cmdText, connection))
                        {
                            ...
                            using (SqlDataReader reader = command.ExecuteReader(CommandBehavior.SingleRow))
                            {
                                ...
                                str = reader["PasswordHash"] as string;
                                str2 = reader["PasswordSalt"] as string;
                                if (this.CheckOldMD5Password)
                                {
                                    flag = (bool)reader["MD5PasswordFlag"];
                                }
                            }
                        }
                    }
            ...
            else if (!ComputeSHA512(password + str2).Equals(str, StringComparison.InvariantCultureIgnoreCase))
            {
                return false;
            }
        ...
        /// <returns></returns>
        private bool CheckPasswordAnswer(string username, string answer)
        {
            using (SqlConnection db = this.OpenDatabase())
            using (SqlCommand cmd = new SqlCommand("SELECT s.UserId FROM UserSecurity as s Inner Join Users u on s.UserId = u.UserId WHERE s.Answer =@Answer and u.UserName=@UserName", db))
            {
        ...
        public override string ResetPassword(string username, string answer)
        {
            ...
            bool isChecked = !RequiresQuestionAndAnswer || (CheckPasswordAnswer(username, answer));
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            ...
            try
            {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("usp_Users_Delete", db))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username;
                    return cmd.ExecuteNonQuery() != 0;
        ...
        public override MembershipUserCollection FindUsersByEmail(string emailToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataTable userTable = new DataTable())
            {
                try
                {
                    using (HostingEnvironment.Impersonate())
                    using (SqlConnection db = this.OpenDatabase())
                    using (SqlCommand cmd = new SqlCommand(getUserSql + "WHERE u.Email LIKE @Email", db))
                    {
        ...
        public override MembershipUserCollection FindUsersByName(string usernameToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataTable userTable = new DataTable())
            {
                try
                {
                    using (HostingEnvironment.Impersonate())
                    using (SqlConnection db = this.OpenDatabase())
                    using (SqlCommand cmd = new SqlCommand(getUserSql + "WHERE u.UserName LIKE @UserName", db))
                    {
                        cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = usernameToMatch;
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd)) da.Fill(userTable);
                    }
                }
                catch { throw; } // Security context hack for HostingEnvironment.Impersonate
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataTable userTable = new DataTable())
            {
                try
                {
                    using (HostingEnvironment.Impersonate())
                    using (SqlConnection db = this.OpenDatabase())
                    using (SqlDataAdapter da = new SqlDataAdapter(getUserSql, db))
                    {
                        da.Fill(userTable);
                    }
                }
        ...
        public bool CheckUserExists(string username)
        {
            ...
            try
            {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("SELECT COUNT(UserId) FROM Users WHERE UserName=@UserName", db))
                {
                    cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username.ToLower();
        ...
        private MembershipUserCollection GetUsersFromDataTable(DataTable userTable, int startIndex, int endIndex)
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                DateTime lastActivityDate = new DateTime(); if (userTable.Rows[i]["DateLastActivity"] != DBNull.Value) lastLoginDate = System.Convert.ToDateTime(userTable.Rows[i]["DateLastActivity"]);
        ...
        ///// <summary>
        ///// Checks if the user password is md5.
        ///// </summary>
        ///// <param name="username">The username.</param>
        //private bool CheckIsMd5UserPassword(string username)
        //{
        //    if (string.IsNullOrEmpty(username)) return false;
        //    if (username.Length > 100) throw new ArgumentOutOfRangeException("username", "Maximum length of 100 characters exceeded");
        ...
        //        using (SqlConnection db = this.OpenDatabase())
        //        {
        //            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(U.UserId) FROM Users U INNER JOIN UserSecurity S ON S.UserId =U.UserId  WHERE UserName=@UserName AND " + MD5PasswordFlagStr + " = 1", db))
        //            {
        //                cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username.ToLower();
        //                return (int)cmd.ExecuteScalar() == 1;
        //            }
        ...
        private bool SetPassword(string username, string password)
        {
            ...
            try
            {
                ...
                using (SqlConnection db = this.OpenDatabase())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(cmdText, db))
                    {
                        cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username;
                        cmd.Parameters.Add("@PasswordHash", SqlDbType.Char, 86).Value = passwordHash;
                        cmd.Parameters.Add("@PasswordSalt", SqlDbType.Char, 5).Value = passwordSalt;
        ...
        private void UpdateLastLoginDate(string username)
        {
            ...
            try
            {
                ...
                using (SqlConnection db = this.OpenDatabase())
                {
                    ...
                    if (RequiresUniqueEmail && IsEmail(username))
                    {
                        cmdText = "UPDATE Users SET DateLastLogin=GETDATE(), DateLastActivity=GETDATE() WHERE Email=@UserName";
                    }
                    else
                    {
                        cmdText = "UPDATE Users SET DateLastLogin=GETDATE(), DateLastActivity=GETDATE() WHERE UserName=@UserName";
                    }
                    using (SqlCommand cmd = new SqlCommand(cmdText, db))
                    {
                        cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = username.ToLower();
