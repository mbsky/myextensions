...
namespace MyExtensions.Web.Security
{
    ...
    public sealed class SimpleSqlRoleProvider : RoleProvider {
        ...
        public override string[] GetRolesForUser(string username) {
            ...
            // Get data from database
            try {
                using (HostingEnvironment.Impersonate()) using (DataTable roleTable = new DataTable())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("usp_UsersInRoles_GetRolesForUser", db))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public override bool IsUserInRole(string username, string rolename) {
            ...
            try {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("usp_UsersInRoles_IsUserInRole", db))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@RoleName", SqlDbType.NVarChar, 100).Value = rolename;
                    cmd.Parameters.Add("@UserName", SqlDbType.VarChar, 100).Value = username;
        ...
        public override void RemoveUsersFromRoles(string[] usernames, string[] rolenames) {
            ...
            try {
                using (HostingEnvironment.Impersonate())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("usp_UsersInRoles_Delete", db))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@UserName", SqlDbType.VarChar, 100);
                    cmd.Parameters.Add("@RoleName", SqlDbType.NVarChar, 100);
        ...
        public override string[] FindUsersInRole(string rolename, string usernameToMatch) {
            ...
            try {
                ...
                using (DataTable roleTable = new DataTable())
                using (SqlConnection db = this.OpenDatabase())
                using (SqlCommand cmd = new SqlCommand("usp_UsersInRoles_FindUsersInRole", db))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@RoleName", SqlDbType.NVarChar, 100).Value = rolename;
